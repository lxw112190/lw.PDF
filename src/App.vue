...
function onDragEnter(event: DragEvent) { if (!hasDroppedFiles(event)) return; event.preventDefault(); dragDepth++; dropActive.value = true }
function onDragOver(event: DragEvent) { if (!hasDroppedFiles(event)) return; event.preventDefault(); if (event.dataTransfer) event.dataTransfer.dropEffect = 'copy' }
function onDragLeave(event: DragEvent) { if (!hasDroppedFiles(event)) return; event.preventDefault(); dragDepth = Math.max(0, dragDepth - 1); if (!dragDepth) dropActive.value = false }
async function onDrop(event: DragEvent) { if (!hasDroppedFiles(event)) return; event.preventDefault(); dragDepth = 0; dropActive.value = false; const file = Array.from(event.dataTransfer?.files ?? []).find(isPdfFile); if (!file) { viewerState.error = '仅支持拖入 PDF 文件。'; return }; try { await viewer.value?.open(await openBrowserPdf(file)) } catch { viewerState.error = '无法打开拖入的 PDF 文件。' } }
